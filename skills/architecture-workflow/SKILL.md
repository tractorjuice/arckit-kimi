...
description: This skill should be used when the user is starting an architecture project
  ...
  do I begin', 'help me get started', 'which /skill:arckit-* in what order', 'set up a new
...
<HARD-GATE>
Do NOT run any `/skill:arckit-*` commands during this process. Your only output is a recommended command plan. The user decides when and what to execute. This applies regardless of how simple the project seems.
</HARD-GATE>
...
Phase 1: Foundation
  1. /skill:arckit-principles — Governance foundation (GDS + TCoP aligned)
  2. /skill:arckit-stakeholders — Map DDaT roles, SROs, policy owners
  3. /skill:arckit-risk — HMG Orange Book risk methodology

Phase 2: Business Justification
  4. /skill:arckit-sobc — HM Treasury Green Book 5-case model
  5. /skill:arckit-requirements — Central artifact for all downstream work

Phase 3: Design & Analysis
  6. /skill:arckit-datascout — Discover UK Gov open data sources
  7. /skill:arckit-data-model — Data architecture with GDPR considerations
  ...
...
This skill is invoked by the `/skill:arckit-start` command, which delegates project onboarding to this skill. Users can also trigger it by asking about getting started, command sequences, or workflow recommendations.
