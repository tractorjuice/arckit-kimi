...
2. **Read Available Documents**:
   ...
   **MANDATORY** (warn if missing):
   - **AAGI** (Agent Inventory) — Extract: Agent list, capabilities, risk levels
     - If missing: warn user to run `/skill:arckit-agent-inventory` first
   - **AAGR** (Agent Design) — Extract: Design patterns, decision-making autonomy, integration points
     - If missing: warn user to run `/skill:arckit-agent-design` first
   ...
   **Read the template** (with user override support):
   ...
   > **Tip**: Users can customize templates with `/skill:arckit-customize agent-governance`
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/skill:arckit-agent-governance` command | [PENDING] | [PENDING] |
```
...
```markdown
**Generated by**: ArcKit `/skill:arckit-agent-governance` command
...
User: `/skill:arckit-agent-governance research-agent`
...
- Governance framework feeds into agent security requirements — handoff to `/skill:arckit-agent-security` after completion
...
- `/skill:arckit-agent-security` -- Align security controls with governance requirements
