...
- **SEC** or **SBD** (Secure by Design) — Extract: Existing security controls, threat models, security standards
  - If missing: warn user to run `/skill:arckit-secure` or `/skill:arckit-mod-secure` first
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/skill:arckit-agent-security` command | [PENDING] | [PENDING] |
```
...
```markdown
**Generated by**: ArcKit `/skill:arckit-agent-security` command
...
User: `/skill:arckit-agent-security research-agent`
...
- Security architecture feeds into governance requirements — handoff to `/skill:arckit-agent-governance` after completion
...
- `/skill:arckit-agent-governance` -- Align security controls with governance requirements
