...
- **ADMP** (Architecture Vision / Preliminary ADM) — Extract: Scope boundaries, ADM phase inclusions, strategic vision
  - If missing: STOP and ask user to run `/skill:arckit-adm-preliminary` first. The application inventory must be scoped by the architecture vision.
...
> **Tip**: Users can customise templates with `/skill:arckit-customize application-inventory`
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/skill:arckit-application-inventory` command |
```

#### Generation Metadata Footer

```markdown
**Generated by**: ArcKit `/skill:arckit-application-inventory` command
**Generated on**: {DATE}
...
2. Run `/skill:arckit-application-rationalization` to decide keep/merge/replace/retire
3. Run `/skill:arckit-gap-analysis` to identify capability coverage gaps
...
6. **Integration with Other Commands**:
   ...
   - **Output**: Feeds `/skill:arckit-application-rationalization` (portfolio decisions), `/skill:arckit-gap-analysis` (capability gaps)
...
- `/skill:arckit-application-rationalization` -- Decide keep/merge/replace/retire for each application
- `/skill:arckit-gap-analysis` -- Analyze capability coverage from application inventory
