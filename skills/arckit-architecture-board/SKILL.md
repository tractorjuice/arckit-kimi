...
- **PRIN** (Architecture Principles, in `000-global`) — Extract: All principles, governance standards, decision rights, compliance requirements
  - If missing: STOP and ask user to run `/skill:arckit-principles` first. The Architecture Board cannot operate without defined principles.
...
> **Tip**: Users can customise templates with `/skill:arckit-customize architecture-board`
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/skill:arckit-architecture-board` command | [PENDING] | [PENDING] |
```
...
```markdown
**Generated by**: ArcKit `/skill:arckit-architecture-board` command
...
3. Create architecture change requests: `/skill:arckit-architecture-change`
...
7. **Integration with other commands**:
   - Architecture Board Charter enables: `/skill:arckit-architecture-change` (governed change requests), `/skill:arckit-gap-analysis` (board prioritises gaps)
   - Architecture Board Charter is informed by: `/skill:arckit-principles` (PRIN), `/skill:arckit-adm-preliminary` (ADMP)
...
- `/skill:arckit-architecture-change` -- Create architecture change requests
