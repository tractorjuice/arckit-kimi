...
- **PRIN** (Architecture Principles, in 000-global) — Extract: Guiding principles, decision framework, technology standards, compliance requirements
  - If missing: STOP and ask user to run `/skill:arckit-principles` first. The repository must be grounded in architecture principles.
...
> **Tip**: Users can customise templates with `/skill:arckit-customize architecture-repository`
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/skill:arckit-architecture-repository` command | [PENDING] | [PENDING] |
```

**Populate Generation Metadata Footer**:

```markdown
**Generated by**: ArcKit `/skill:arckit-architecture-repository` command
...
3. Feed repository standards into next ADM cycle: `/skill:arckit-adm-preliminary`
4. Add new patterns as projects complete — re-run `/skill:arckit-architecture-repository`
...
5. **Living Document**: The Architecture Repository is a living artefact. Schedule quarterly reviews (`/skill:arckit-architecture-repository`) to incorporate learnings from new projects. Version management (v1.0 → v2.0) tracks evolution.
...
7. **Integration with Other Commands**:
   ...
   - **Output**: Feeds into `/skill:arckit-adm-preliminary` (standards inform scope), all future project artefacts (reference patterns and standards)
...
- `/skill:arckit-adm-preliminary` -- Feed repository standards back into next ADM cycle
