...
The Australian Energy Sector Cyber Security Framework is an energy-sector cyber maturity framework coordinated through AEMO and industry partners. Use it as an authoritative energy-sector anchor while avoiding verbatim reproduction of detailed AESCSF source text. This artefact complements `/skill:arckit-au-e8-posture`, `/skill:arckit-au-ism-controls`, `/skill:arckit-au-ot-security`, and `/skill:arckit-au-soci-cirmp` by translating baseline cyber, OT, and critical-infrastructure evidence into an energy-specific maturity view.
...
6. Generate the following sections:
   ...
   - **Asset, Interface, and Evidence Inventory** - identify source-of-truth registers for OT assets, IT systems, DERMS / DOE platforms, market interfaces, APIs, telemetry links, metering assets, vendor remote-access paths, data catalogues, control evidence, owners, criticality, and inventory gaps. Where ServiceNow or another CMDB exists, cross-reference `/skill:arckit-servicenow` output and ensure inventory rows can be visualised with colour-coded criticality, maturity, or risk status.

   - **Diagram and Traceability Handoffs** - list required `/skill:arckit-dfd`, `/skill:arckit-diagram`, `/skill:arckit-data-model`, `/skill:arckit-traceability`, and `/skill:arckit-risk` updates.
   ...
   - **Federal Baseline Cross-Reference** - map relevant findings to `/skill:arckit-au-e8-posture`, `/skill:arckit-au-ism-controls`, `/skill:arckit-au-ot-security`, and `/skill:arckit-au-soci-cirmp`.
...
- Where the project includes critical infrastructure, cross-reference `/skill:arckit-au-soci-cirmp`; where it includes connected OT, cross-reference `/skill:arckit-au-ot-security`.
- Inventory-heavy findings should reuse existing ArcKit artefacts: `/skill:arckit-data-model` for data catalogues, `/skill:arckit-servicenow` for CMDB/service inventory, `/skill:arckit-dfd` and `/skill:arckit-diagram` for colour-coded visualisation, `/skill:arckit-risk` for heat/scoring, `/skill:arckit-maturity-model` for maturity criteria, and `/skill:arckit-graph-report` for coverage gaps.

## Suggested Next Steps

After completing this command, consider running:

- `/skill:arckit-au-e8-posture` -- Essential Eight posture provides the enterprise cyber baseline for AESCSF maturity evidence.
- `/skill:arckit-au-ism-controls` -- ISM applicability provides control evidence and government security cross-references for AESCSF gaps.
- `/skill:arckit-au-ot-security` -- OT security findings provide zone, conduit, connectivity, and remote-access evidence for energy OT environments.
- `/skill:arckit-au-soci-cirmp` -- SOCI/CIRMP evidence supports critical-infrastructure risk treatment and incident escalation context.
- `/skill:arckit-dfd` -- Data-flow diagrams provide IT/OT, market, DERMS, DOE, CSIP-AUS, and vendor-access evidence.
- `/skill:arckit-diagram` -- Architecture diagrams provide OT/IT zone, conduit, ring, market, and grid-edge context.
- `/skill:arckit-data-model` -- Energy data-model dependencies identify asset, metering, settlement, DER, telemetry, and customer data impacts.
- `/skill:arckit-servicenow` -- CMDB and service inventory evidence maps architecture components, dependencies, owners, and support criticality.
- `/skill:arckit-maturity-model` -- Maturity scoring can calibrate AESCSF capability levels and evidence thresholds.
- `/skill:arckit-traceability` -- Traceability links AESCSF findings to requirements, obligations, controls, decisions, and evidence.
- `/skill:arckit-risk` -- AESCSF maturity gaps and residual exposures should feed the project risk register.
- `/skill:arckit-graph-report` -- Governance graph metrics show coverage, cross-reference density, and missing evidence links across registers.
