...
6. Generate the following sections:
   ...
   - **Energy-Specific SOCI Interpretation** - cross-reference `/skill:arckit-au-soci-cirmp` for critical-infrastructure applicability, protected information, reporting, CIRMP hazards, and SOCI incident escalation.
   ...
   - **Regulated Asset, Interface, and Data Inventory** - identify source-of-truth registers for regulated services, unregulated services, critical assets, CMDB CIs, AEMO interfaces, APIs, market files, telemetry links, vendor access paths, customer data, metering data, settlements, owners, criticality, and inventory gaps. Cross-reference `/skill:arckit-servicenow`, `/skill:arckit-data-model`, `/skill:arckit-dfd`, `/skill:arckit-diagram`, `/skill:arckit-risk`, and `/skill:arckit-traceability` where available.
   ...
   - **Diagram and Traceability Handoffs** - list required `/skill:arckit-dfd`, `/skill:arckit-diagram`, `/skill:arckit-data-model`, and `/skill:arckit-traceability` updates.
...
- Use `/skill:arckit-au-aescsf` for energy cyber maturity and `/skill:arckit-au-ot-security` for OT-specific evidence where applicable.
- Inventory-heavy findings should reuse existing ArcKit artefacts: `/skill:arckit-data-model` for data catalogues, `/skill:arckit-servicenow` for CMDB/service inventory, `/skill:arckit-dfd` and `/skill:arckit-diagram` for colour-coded visualisation, `/skill:arckit-risk` for heat/scoring, and `/skill:arckit-graph-report` for register coverage gaps.

## Suggested Next Steps

After completing this command, consider running:

- `/skill:arckit-au-soci-cirmp` -- SOCI/CIRMP evidence provides critical-infrastructure obligations and incident escalation context.
- `/skill:arckit-au-aescsf` -- AESCSF maturity findings provide energy-sector cyber posture evidence.
- `/skill:arckit-au-ot-security` -- OT security evidence supports control-system, grid-edge, and operational technology compliance context.
- `/skill:arckit-au-pia` -- Privacy impact evidence supports customer, metering, billing, and energy data handling.
- `/skill:arckit-dfd` -- Data-flow diagrams provide regulated/unregulated, market, AEMO, metering, DER, and customer flow evidence.
- `/skill:arckit-diagram` -- Architecture diagrams provide ring-fencing boundaries, interface maps, and operational context.
- `/skill:arckit-data-model` -- The energy data model identifies regulated, unregulated, customer, metering, settlement, and operational data dependencies.
- `/skill:arckit-servicenow` -- CMDB and service inventory evidence maps regulated services, CIs, dependencies, owners, support tiers, and operational criticality.
- `/skill:arckit-traceability` -- Traceability links obligations to requirements, controls, decisions, evidence, and residual gaps.
- `/skill:arckit-adr` -- Architecture decisions should be raised for ring-fencing boundaries, AEMO interfaces, DER integration, and data-sharing choices.
- `/skill:arckit-graph-report` -- Governance graph metrics show register coverage, cross-reference density, and missing compliance evidence links.
