...
6. Generate the following sections:
   ...
   - **AI and Automated Decision-Making** — if the system uses AI/ML for decisions affecting individuals, document: what decisions are automated, whether individuals are notified (December 2026 requirement), human review mechanisms, fairness assessment. Cross-reference `/skill:arckit-au-ai-assurance` if applicable.

   - **Recommendations** — prioritised list of privacy-enhancing measures.

   - **ArcKit Evidence Integration** — map `/skill:arckit-dfd`, `/skill:arckit-data-model`, `/skill:arckit-risk`, `/skill:arckit-traceability`, and `/skill:arckit-graph-report` outputs to APP assessments, personal-information flows, privacy risks, mitigation ownership, and coverage gaps.
...
- `/skill:arckit-dfd` -- DFDs identify personal-information collection, use, disclosure, cross-border transfer, and retention flows.
- `/skill:arckit-data-model` -- Data model evidence identifies APP-relevant entities, sensitive information, identifiers, owners, retention, and access controls.
- `/skill:arckit-au-dss` -- PIA findings feed DSS Criterion 7 (Protect users' privacy).
- `/skill:arckit-au-e8-posture` -- APP 11 (security of personal information) informs E8 target maturity level.
- `/skill:arckit-risk` -- Privacy risks surface in the project risk register.
- `/skill:arckit-traceability` -- APP obligations and privacy mitigations should trace to requirements, data entities, risks, and controls.
- `/skill:arckit-graph-report` -- Graph reporting should show AUPIA coverage alongside data-model, risk, traceability, and AU compliance artefacts.
