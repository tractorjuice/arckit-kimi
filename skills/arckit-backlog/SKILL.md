...
- **REQ** (Requirements) — primary source
  ...
  - If missing: warn user to run `/skill:arckit-requirements` first — backlog is derived from requirements
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/skill:arckit-backlog` command | [PENDING] | [PENDING] |
```
...
```markdown
**Generated by**: ArcKit `/skill:arckit-backlog` command
...
📚 Integration:
  ...
  - Link to /skill:arckit-traceability for requirements tracking
```
...
Then re-run /skill:arckit-backlog
```
...
For better component mapping, run:
  /skill:arckit-hld or /skill:arckit-diagram
...
```bash
/skill:arckit-backlog VELOCITY=25 SPRINTS=12
```
...
```bash
/skill:arckit-backlog FORMAT=all
```
...
```bash
/skill:arckit-backlog PRIORITY=risk
```
...
- `/skill:arckit-requirements` → All stories
- `/skill:arckit-hld` → Component mapping
- `/skill:arckit-stakeholders` → User personas
- `/skill:arckit-risk-register` → Risk priorities
- `/skill:arckit-threat-model` → Security stories
- `/skill:arckit-business-case` → Value priorities
- `/skill:arckit-principles` → Definition of Done

### Outputs To

- `/skill:arckit-traceability` → Requirements → Stories → Sprints
- `/skill:arckit-test-strategy` → Test cases from acceptance criteria
- `/skill:arckit-analyze` → Backlog completeness check
...
- `/skill:arckit-trello` -- Export backlog to Trello board
- `/skill:arckit-traceability` -- Map user stories back to requirements
