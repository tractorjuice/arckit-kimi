...
- **ADMP** (ADM Preliminary / Architecture Vision) — Extract: Scope boundaries, architecture vision, drivers, constraints, success criteria, in-scope capabilities
  - If missing: STOP and ask user to run `/skill:arckit-adm-preliminary` first. Business Architecture must be grounded in the ADM scope and vision.
...
> **Tip**: Users can customise templates with `/skill:arckit-customize capability-map`
...
4. Perform gap analysis: `/skill:arckit-gap-analysis`
5. Map applications to capabilities: `/skill:arckit-application-inventory`
...
7. **Integration with Other Commands**:
   - BPCM feeds into: `/skill:arckit-gap-analysis` (Phase E — gap identification), `/skill:arckit-application-inventory` (Phase C — application architecture)
   - BPCM is informed by: `/skill:arckit-adm-preliminary`, `/skill:arckit-requirements`, `/skill:arckit-stakeholders`, `/skill:arckit-principles`
...
- `/skill:arckit-gap-analysis` -- Analyze capability gaps against target state
- `/skill:arckit-application-inventory` -- Map applications to capabilities
