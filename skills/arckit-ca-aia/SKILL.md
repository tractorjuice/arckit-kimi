...
- `/skill:arckit-risk` -- AIA findings — bias, drift, contestability — feed the operational risk register.
- `/skill:arckit-adr` -- Material AIA outcomes (vendor selection, autonomy tier, recourse design) warrant Architecture Decision Records.
- `/skill:arckit-ca-pia` -- Personal-information feeding the algorithmic system inherits PIA controls; AIA depth must match PIA depth.
