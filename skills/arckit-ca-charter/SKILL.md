...
- `/skill:arckit-ca-fitaa` -- Charter §2 expression and association analysis is a mandatory companion to FITAA; the FITAA artefact references this review.
- `/skill:arckit-ca-pia` -- §8 search-and-seizure analysis is grounded in the personal-information categories captured in the Privacy Impact Assessment.
- `/skill:arckit-risk` -- Residual Charter risks per right feed the operational risk register with appropriate severity.
