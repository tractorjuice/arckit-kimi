...
- `/skill:arckit-ca-charter` -- Charter §2 expression and association analysis is required for any registration scheme that engages protected speech or association.
- `/skill:arckit-ca-pia` -- Privacy Impact Assessment for personal information collected during arrangement registration.
- `/skill:arckit-ca-atip` -- Reconciles the public-facing register against the protected investigative dataset; required severance design for hybrid views.
- `/skill:arckit-ca-aia` -- Triggered when registration triage uses any automated decision-making, scoring, or risk classification.
