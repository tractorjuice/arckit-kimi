...
This is the supplier-centric sibling of `/skill:arckit-tenders`: it shares the same
reader (`arckit-tenders-reader`) and the same handoff schema
(`tenders-handoff.schema.json`). Where `/skill:arckit-tenders` frames the data as
...
1. Dispatch the reader using the `Agent` tool with
   ...
   prompt. (This is the **shared** reader — the same one `/skill:arckit-tenders`
   dispatches.)
...
- Next steps (`/skill:arckit-research`, `/skill:arckit-score`, `/skill:arckit-risk`).
...
- `/skill:arckit-research` -- Feed the competitive set into build-vs-buy analysis
- `/skill:arckit-score` -- Use rival award history as Company Experience evidence
- `/skill:arckit-risk` -- Record supplier-concentration / single-supplier-dependency risk
