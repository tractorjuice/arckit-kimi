...
- If NOT found: ERROR "Run /skill:arckit-principles first to define governance standards for your organization"

### Architecture Decision Records (MANDATORY)

b. **ADR** (Architecture Decision Records, in `projects/{project-dir}/decisions/`) (MUST exist):

- If NOT found: ERROR "Run /skill:arckit-adr first — conformance assessment requires at least one accepted ADR"
...
- If NOT found: ERROR "Architecture principles not found. Run /skill:arckit-principles first."

**Check ADRs**:

- Look for `ARC-*-ADR-*.md` files in `projects/{project-dir}/decisions/`
- If NONE found: ERROR "No ADRs found. Run /skill:arckit-adr first — conformance assessment requires at least one accepted ADR."
...
**Note**: This differs from `/skill:arckit-principles-compliance` which provides RAG scoring with remediation plans. This check is a binary gate: does the design conform or not?
...
1. **Suggest Follow-up Commands**:
   ...
   ```text
   📋 **Related Commands**:
   - /skill:arckit-principles-compliance - Detailed RAG scoring of principle compliance
   - /skill:arckit-analyze - Comprehensive governance gap analysis
   - /skill:arckit-traceability - Requirements traceability matrix
   - /skill:arckit-health - Quick metadata health check
   ```
