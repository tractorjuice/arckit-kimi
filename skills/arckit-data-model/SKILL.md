...
1. **Read existing artifacts from the project context:**
   ...
   **MANDATORY** (warn if missing):
   - **REQ** (Requirements)
     ...
     - If missing: STOP and warn user to run `/skill:arckit-requirements` first — data model MUST be based on DR-xxx requirements
...
4. **Read the template** (with user override support):
   ...
   > **Tip**: Users can customize templates with `/skill:arckit-customize data-model`
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/skill:arckit-data-model` command |
```

### Generation Metadata Footer

```markdown
**Generated by**: ArcKit `/skill:arckit-data-model` command
**Generated on**: {DATE}
...
- Suggested next steps (e.g., "Review data model with data protection officer before proceeding to HLD" or "Run `/skill:arckit-hld-review` to validate database technology choices")

## Example Usage

User: `/skill:arckit-data-model Create data model for payment gateway project`
...
- **Output**: Feeds into `/skill:arckit-hld-review` (validates database technology choices)
- **Output**: Feeds into `/skill:arckit-dld-review` (validates schema design, indexes, query patterns)
- **Output**: Feeds into `/skill:arckit-sow` (RFP includes data migration, data governance requirements)
- **Output**: Supports `/skill:arckit-traceability` (DR-xxx → Entity → Attribute → HLD Component)
...
- Run `/skill:arckit-research` to research database technologies
- Run `/skill:arckit-hld-review` after HLD is created
```
...
- `/skill:arckit-hld-review` -- Validate database technology choices
- `/skill:arckit-dld-review` -- Validate schema design and query patterns
- `/skill:arckit-sow` -- Include data migration and governance in RFP
- `/skill:arckit-traceability` -- Map DR-xxx to entities and attributes
