...
- User specified type only (e.g., `/skill:arckit-diagram context`): skip Question 1, **still ask Question 2**
- User specified format only (e.g., `/skill:arckit-diagram plantuml`): skip Question 2, still ask Question 1
- User specified both (e.g., `/skill:arckit-diagram context plantuml`): skip both questions
...
> **Tip**: Users can customize templates with `/skill:arckit-customize diagram`
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/skill:arckit-diagram` command | [PENDING] | [PENDING] |
```
...
```markdown
**Generated by**: ArcKit `/skill:arckit-diagram` command
...
"I recommend running `/skill:arckit-requirements` first to establish requirements before creating diagrams."

# If no HLD exists
"For a container diagram, I recommend having an HLD first. Run `/skill:arckit-hld-review` or create HLD documentation."

# If no Wardley Map exists
"For strategic context (build vs buy), consider running `/skill:arckit-wardley` first."
```
...
- Run `/skill:arckit-hld-review` to create technical architecture
- Run `/skill:arckit-diagram container` to show technical containers"

# After Container diagram
"Your container diagram is ready. Next steps:
- Run `/skill:arckit-dld-review` for detailed component design
- Run `/skill:arckit-diagram component` to show internal structure
- Run `/skill:arckit-diagram deployment` to show infrastructure"

# After Deployment diagram
"Your deployment diagram is ready. Consider:
- Running `/skill:arckit-tcop` to validate Cloud First compliance
...
```bash
"/skill:arckit-hld-review Review HLD with container diagram validation"
```
...
The `/skill:arckit-traceability` command should include diagram references:
...
/skill:arckit-requirements Build a payment gateway...

# 2. Create context diagram (shows system boundary)
/skill:arckit-diagram context Generate context diagram for payment gateway

# 3. Create Wardley Map (strategic positioning)
/skill:arckit-wardley Create Wardley Map showing build vs buy

# 4. Create HLD
/skill:arckit-hld-review Create high-level design

# 5. Create container diagram (technical architecture)
/skill:arckit-diagram container Generate container diagram showing technical architecture
```
...
/skill:arckit-diagram container Generate container diagram from Acme Corp HLD

# 3. Review HLD with diagram validation
/skill:arckit-hld-review Review Acme Corp HLD against container diagram
```
...
/skill:arckit-diagram component Generate component diagram for Payment API container

# 2. Create sequence diagram for key flows
/skill:arckit-diagram sequence Generate sequence diagram for payment processing flow

# 3. Review DLD
/skill:arckit-dld-review Review detailed design with component and sequence diagrams
```
...
/skill:arckit-diagram deployment Generate AWS deployment diagram showing Cloud First compliance

# 2. Create data flow diagram
/skill:arckit-diagram dataflow Generate data flow diagram showing UK GDPR PII handling

# 3. Assess TCoP compliance
/skill:arckit-tcop Assess TCoP compliance with deployment and data flow diagrams
```
...
- /skill:arckit-hld-review - Review HLD against this diagram
- /skill:arckit-traceability - Validate requirements coverage
- /skill:arckit-analyze - Comprehensive governance quality check
```
