...
- **DATA** (Data Model) — Extract: all entities with PII/special category data, data subjects, GDPR Article 6 lawful basis, Article 9 conditions, retention periods, data flows, data classifications
  - If missing: STOP and warn user to run `/skill:arckit-data-model` first — a DPIA requires a data model to identify personal data processing
...
> **Tip**: Users can customize templates with `/skill:arckit-customize dpia`
...
11. **AI/ML Systems**: If the system uses AI/ML for profiling, automated decision-making, or algorithmic processing, integrate with `/skill:arckit-ai-playbook` assessment and link to ATRS record.
...
```text
/skill:arckit-dpia Generate DPIA for NHS appointment booking system

/skill:arckit-dpia Create data protection impact assessment for HMRC chatbot handling taxpayer queries

/skill:arckit-dpia Assess DPIA necessity for Windows 11 deployment (employee data only)
```
