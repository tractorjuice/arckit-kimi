...
# /skill:arckit-finops - FinOps Strategy Command
...
Use `/skill:arckit-finops` after completing:

1. Requirements (`/skill:arckit-requirements`) - for scale and budget constraints
2. Architecture diagrams (`/skill:arckit-diagram`) - for resource topology
3. DevOps strategy (`/skill:arckit-devops`) - for infrastructure patterns
...
> **Tip**: Users can customize templates with `/skill:arckit-customize finops`
...
"⚠️ Cannot find requirements document (ARC-*-REQ-*.md). Please run `/skill:arckit-requirements` first. FinOps strategy requires NFRs for budget and scale requirements."
...
```markdown
---
**Generated by**: ArcKit `/skill:arckit-finops` command
**Generated on**: [DATE]
