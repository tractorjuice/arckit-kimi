...
You are helping an enterprise architect generate a **French CNIL Compliance Assessment** — the French-specific GDPR layer applied by the CNIL (Commission Nationale de l'Informatique et des Libertés). Run this after `/skill:arckit-eu-rgpd` to add French obligations that go beyond the EU GDPR baseline.
...
- **RGPD** (EU RGPD Assessment) — Extract: legal basis mapping, DPIA screening results, DPO determination, international transfer analysis
  - If missing: warn that `/skill:arckit-fr-rgpd` should be run after `/skill:arckit-eu-rgpd` for best results. Proceed with available data.
...
5. **Section 3: Health Data** (only if health data detected in data model or user input)
   ...
   - DPIA mandatory flag: health data = special category → run `/skill:arckit-dpia`
...
Next steps:
1. {If health data: Run /skill:arckit-fr-secnumcloud for HDS-compliant hosting}
2. {If DPIA required: Run /skill:arckit-dpia}
3. {If procurement: Run /skill:arckit-fr-marche-public for DPA clauses}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

## Important Notes

- **Run after eu-rgpd**: This command adds the French layer on top of the EU GDPR baseline. For best results, run `/skill:arckit-eu-rgpd` first, then this command.
...
> **Note for reviewers**: This command covers France-specific GDPR obligations layered on top of the baseline EU GDPR (covered by `/skill:arckit-eu-rgpd`). Key French specifics: the age of digital consent is **15** (not the GDPR default of 16), HDS (Hébergement de Données de Santé) is a mandatory French certification for any cloud provider hosting health data, and the CNIL has issued specific guidance on analytics tools — notably ruling that Google Analytics transfers personal data to the US unlawfully (2022). The CNIL is the French Data Protection Authority (DPA), member of the EDPB.
...
```text
/skill:arckit-fr-rgpd Assess CNIL compliance for a French regional hospital group deploying a patient portal, processing données de santé, with third-party analytics and a mobile app targeting both adults and teenagers

/skill:arckit-fr-rgpd CNIL layer for 001 — e-commerce platform with Google Analytics, loyalty profiling, EU-US transfers

/skill:arckit-fr-rgpd French GDPR layer for a ministry HR system handling agent personal data, DPO mandatory, no health data
```
...
- `/skill:arckit-dpia` -- Run a full Data Protection Impact Assessment if CNIL screening flags high risk *(when 2+ CNIL DPIA criteria triggered)*
- `/skill:arckit-fr-secnumcloud` -- Assess SecNumCloud requirements for health data hosting *(when Health data (données de santé) processed — HDS hosting required)*
- `/skill:arckit-fr-marche-public` -- Include GDPR/DPA obligations in procurement documentation *(when Procurement involves data processors)*
