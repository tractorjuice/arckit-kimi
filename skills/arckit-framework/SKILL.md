...
**Generated by**: ArcKit `/skill:arckit-framework` agent
**Generated on**: {DATE}
...
Include the generation metadata footer (same format as FWRK overview but referencing `/skill:arckit-framework` agent).
...
- Next steps (suggest `/skill:arckit-pages` to publish, or additional commands to fill gaps in coverage)
...
- **No requirements found**: Note this gap prominently in the Executive Summary. Suggest running `/skill:arckit-requirements`
- **No principles found**: Note this gap. Suggest running `/skill:arckit-principles`
...
- **Related commands** — `/skill:arckit-navigator` (project coverage) · `/skill:arckit-traceability` (cross-reference validation) · `/skill:arckit-strategy` (executive synthesis)
...
- `/skill:arckit-glossary` -- Generate glossary of framework terminology
- `/skill:arckit-maturity-model` -- Create maturity model for framework adoption
