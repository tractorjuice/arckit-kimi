...
- **BPCM** (Business Capability Model) — Extract: Capability hierarchy, maturity levels, capability ownership, capability-to-objective mappings
  - If missing: STOP and ask user to run `/skill:arckit-business-capability-map` first. Gap analysis requires a capability baseline.
...
### Next Steps
1. Review gap analysis with Architecture Board: `/skill:arckit-architecture-board`
2. Create work packages to close gaps: `/skill:arckit-transition-architecture`
...
7. **Integration with Other Commands**:
   - Gap Analysis feeds into: `/skill:arckit-transition-architecture` (Phase F — Migration Planning), `/skill:arckit-architecture-board` (governance review)
   - Gap Analysis is informed by: `/skill:arckit-business-capability-map` (BPCM), `/skill:arckit-strategy` (STRAT), `/skill:arckit-principles` (PRIN)
...
- `/skill:arckit-transition-architecture` -- Create work packages to close identified gaps
- `/skill:arckit-architecture-board` -- Present gap analysis to Architecture Board for prioritization
