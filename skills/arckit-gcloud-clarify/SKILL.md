...
- **REQ** (Requirements) — Extract: All MUST requirements (BR-xxx, FR-xxx, NFR-xxx, INT-xxx, DR-xxx), SHOULD requirements, compliance (NFR-C-xxx), integration (INT-xxx), performance (NFR-P-xxx), security (NFR-S-xxx)
  - If missing: ERROR "Run `/skill:arckit-requirements` first — need source requirements"
- **GCLD** (G-Cloud Search, in procurement/) — Extract: Shortlisted services (top 3-5), service names, supplier names, service links, key features, Must-Have Match scores, Desirable Features scores, compliance mentions, pricing
  - If missing: ERROR "Run `/skill:arckit-gcloud-search` first — need service search results"
...
7. ✅ **Final evaluation**:
   - [ ] Use `/skill:arckit-evaluate` to score suppliers
...
**Parallel Activity**: While waiting for responses, prepare evaluation criteria with `/skill:arckit-evaluate`.
...
**Generated**: [DATE]
**Tool**: /skill:arckit-gcloud-clarify
**Next Command**: `/skill:arckit-evaluate` (after supplier responses received)
```
...
- **No ARC-*-REQ-*.md**: ERROR "Requirements not found - run /skill:arckit-requirements first"
- **No gcloud-ARC-*-REQ-*.md**: ERROR "G-Cloud search results not found - run /skill:arckit-gcloud-search first"
- **No services shortlisted**: ERROR "No services to clarify - gcloud-search found no results"
- **All MUST requirements confirmed**: INFO "All MUST requirements confirmed with evidence - minimal clarification needed. Proceed to /skill:arckit-evaluate"

## Integration with Workflow

**Complete G-Cloud Procurement Workflow**:

1. `/skill:arckit-requirements` → Define service needs
2. `/skill:arckit-gcloud-search` → Find services on Digital Marketplace
3. **`/skill:arckit-gcloud-clarify`** → Identify gaps, generate questions
4. *Supplier engagement* → Send questions, receive responses
5. `/skill:arckit-evaluate` → Score suppliers based on responses
