...
- `ARC-*-REQ-*.md` in `projects/{project}/` — Requirements specification
  ...
  - If missing: STOP and report that `/skill:arckit-requirements` must be run first
...
```text
**Generated by**: ArcKit `/skill:arckit-gcp-research` agent
**Generated on**: {DATE}
...
- Next steps (`/skill:arckit-diagram`, `/skill:arckit-secure`, `/skill:arckit-devops`)
...
- **No requirements found**: Stop, tell user to run `/skill:arckit-requirements`
...
- **Related commands** — `/skill:arckit-requirements` (input) · `/skill:arckit-research` (cross-cloud comparison) · `/skill:arckit-aws-research` · `/skill:arckit-azure-research`
...
- `/skill:arckit-diagram` -- Create Google Cloud architecture diagrams
- `/skill:arckit-devops` -- Design Cloud Build pipeline
- `/skill:arckit-finops` -- Create Google Cloud cost management strategy
- `/skill:arckit-adr` -- Record Google Cloud service selection decisions
