...
This is a landscape-level signal, **not** a per-repo security audit: report exposure breadth and the highest-impact advisories, and recommend `/skill:arckit-secure` or `/skill:arckit-risk` for any repo the project intends to adopt. If `vulnerability_exposure` returns no data for a scope (e.g. an org with no indexed SBOMs), note it as a coverage gap rather than "no vulnerabilities".
...
```text
**Generated by**: ArcKit `/skill:arckit-gov-landscape` agent
**Generated on**: {DATE}
...
- Key gaps identified
- Next steps (`/skill:arckit-gov-reuse`, `/skill:arckit-framework`, `/skill:arckit-wardley`)
...
- **Related commands** — `/skill:arckit-gov-reuse` (capability-driven reuse) · `/skill:arckit-gov-code-search` (focused queries)
...
- `/skill:arckit-gov-reuse` -- Assess specific repos for reuse
- `/skill:arckit-framework` -- Incorporate patterns into architecture framework
- `/skill:arckit-wardley` -- Map landscape evolution
