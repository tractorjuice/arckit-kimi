...
- `ARC-*-REQ-*.md` in `projects/{project}/` — Requirements specification
  ...
  - If missing: warn user to run `/skill:arckit-requirements` first, but proceed using `$ARGUMENTS` as the project description
...
- Suggested next steps: `/skill:arckit-sobc` (Economic Case), `/skill:arckit-plan` (project plan), `/skill:arckit-risk` (grant-specific risks)
...
- **Related commands** — `/skill:arckit-requirements` (input) · `/skill:arckit-stakeholders` (input) · `/skill:arckit-sobc` (downstream business case)

## Suggested Next Steps

After completing this command, consider running:

- `/skill:arckit-sobc` -- Feed grant funding data into Economic Case
- `/skill:arckit-plan` -- Create project plan aligned to grant milestones
- `/skill:arckit-risk` -- Add grant-specific risks (rejection, compliance, reporting)
- `/skill:arckit-adr` -- Record funding-mix decisions
