...
If the hook block is missing (unlikely — it always fires for `/skill:arckit-graph-report`), inform the user and stop.
...
▶ Recommended actions
   - <bullet list — for each project with <50% compliance readiness, suggest the top missing /skill:arckit-* command>
```
...
- `/skill:arckit-navigator <project>` — single-project "what's next" GPS, complementary to graph-report.
- `/skill:arckit-analyze <project>` — deep governance analysis for one project (sections, requirements, principles, risks).
- `/skill:arckit-health` — point-in-time health rules across all projects.
