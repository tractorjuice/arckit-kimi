...
Scans the `projects/` directory for all `ARC-*` artifacts and applies nine detection rules to identify governance health issues. Each finding is assigned a severity (HIGH, MEDIUM, or LOW) with a suggested remediation action. The hook also writes `docs/health.json` on every run for dashboard integration (consumed by `/skill:arckit-pages`).
...
[HIGH] STALE-RSCH: {filepath}
  ...
  Action: Re-run /skill:arckit-research to refresh pricing and vendor data
```
...
|---------|---------------------|
| `*api*`, `*swagger*`, `*openapi*` | `/skill:arckit-requirements`, `/skill:arckit-data-model`, `/skill:arckit-diagram` |
| `*schema*`, `*erd*`, `*.sql` | `/skill:arckit-data-model`, `/skill:arckit-data-mesh-contract` |
| `*security*`, `*pentest*`, `*vuln*` | `/skill:arckit-secure`, `/skill:arckit-dpia` |
| `*compliance*`, `*audit*` | `/skill:arckit-tcop`, `/skill:arckit-conformance` |
| `*cost*`, `*pricing*`, `*budget*` | `/skill:arckit-sobc`, `/skill:arckit-finops` |
| `*pipeline*`, `*ci*`, `*deploy*` | `/skill:arckit-devops` |
| `*rfp*`, `*itt*`, `*tender*` | `/skill:arckit-sow`, `/skill:arckit-evaluate` |
| `*risk*`, `*threat*` | `/skill:arckit-risk`, `/skill:arckit-secure` |
| `*policy*`, `*standard*` | `/skill:arckit-principles`, `/skill:arckit-tcop` |
| (default) | `/skill:arckit-requirements`, `/skill:arckit-analyze` |
...
1. [HIGH] Address {count} stale research documents
   Run: /skill:arckit-research for affected projects
...
6. [MEDIUM] Check {count} orphaned requirements
   Run: /skill:arckit-adr for requirements needing architectural decisions
...
7. [MEDIUM] Add traceability to {count} ADRs
   ...
   Run: /skill:arckit-traceability to generate full traceability matrix
...
**Dashboard integration**: Run `/skill:arckit-pages` after the health check to see health data on the governance dashboard.
...
```text
No projects/ directory found. Run /skill:arckit-init to create your first project.
```
...
```bash
/skill:arckit-health PROJECT=001-payment-gateway
```
...
```bash
/skill:arckit-health SEVERITY=HIGH
```
...
```bash
/skill:arckit-health SINCE=2026-06-01
```
...
Re-run this health check periodically during long architecture sessions to surface HIGH severity findings as artifacts change. Kimi does not provide a built-in loop command; use a scheduler if you need automation.
...
- `/skill:arckit-analyze` — health check complements the deeper governance analysis
- Any artifact creation — verify new artifacts don't introduce drift

### Triggers For

- `/skill:arckit-research` — refresh stale RSCH documents
- `/skill:arckit-adr` — create ADRs for orphaned requirements
- `/skill:arckit-traceability` — fix missing traceability links
- `/skill:arckit-hld-review` or `/skill:arckit-dld-review` — follow up on unresolved conditions
...
The health check is a **diagnostic tool**, not a governance artifact. Unlike `/skill:arckit-analyze` which produces a formal analysis report (saved as `ARC-*-ANAL-*.md`), the health check is:
...
Console output is the primary user-facing output. `docs/health.json` is always written as a side effect for dashboard integration (`/skill:arckit-pages`).
