...
4. **Output format** (console only — do NOT create a file):
   ...
   ### Recommended Actions
   1. Re-run `/skill:arckit-secure` to update Secure by Design assessment
   2. Re-run `/skill:arckit-dpia` to update Data Protection Impact Assessment
   3. Review ADR-002 decision rationale against updated requirement
   ```

5. **Recommend specific `/skill:arckit-` commands** for HIGH severity impacts:
   - SECD impacted → `/skill:arckit-secure`
   - DPIA impacted → `/skill:arckit-dpia`
   - TCOP impacted → `/skill:arckit-tcop`
   - HLDR impacted → `/skill:arckit-hld-review`
   - RISK impacted → `/skill:arckit-risk`
   - TRAC impacted → `/skill:arckit-traceability`

6. **If no impacts found**, report that the document has no downstream dependencies. Note this may indicate a traceability gap — suggest running `/skill:arckit-traceability` to check coverage.
...
- `/skill:arckit-traceability` -- Update traceability matrix after impact assessment *(when Impact analysis revealed traceability gaps)*
- `/skill:arckit-health` -- Check health of impacted artifacts *(when Impacted documents may be stale)*
- `/skill:arckit-conformance` -- Re-assess conformance after changes *(when Impact includes architecture design documents)*
