...
- **TBB** (Te Beschermen Belangen determination, from `/skill:arckit-nl-tbb`) — Extract: BIV scores to prioritise which control areas matter most
- **RBCLOUD** (Rijksbreed Cloudbeleid Compliance Assessment, from `/skill:arckit-nl-cloud`) — Extract: hosting model, data location, encryption posture already assessed
...
State the system in scope, the BIV scores if available from `/skill:arckit-nl-tbb`, and the certification posture: BIO2 does not require ISO/IEC 27001 certification, but its overheidsmaatregelen are mandatory where applicable regardless of certification status. Do not present "not certified" as equivalent to "non-conformant" — they are different questions.
...
Cross-reference `/skill:arckit-nl-cloud` Section 7 (data location and encryption) for the Technological controls theme where the system is cloud-hosted — do not re-derive the encryption assessment from scratch if it already exists.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Next steps:
1. {If BIV scores not available: Run /skill:arckit-nl-tbb to prioritise remaining control work}
2. {If cloud-hosted: Run /skill:arckit-nl-cloud for the data-location and encryption assessment}
3. Run /skill:arckit-risk to add open gaps to the risk register
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
- **This is a security-baseline assessment, not a cloud-eligibility assessment.** Run `/skill:arckit-nl-cloud` for hosting eligibility and `/skill:arckit-nl-tbb` for classification.
...
- ✅ BIV scores from `/skill:arckit-nl-tbb` used to prioritise controls where available
...
```text
/skill:arckit-nl-bio Assess BIO2 conformance for a ministry shared-services platform, ISO 27001 certification not pursued, BIV scores not yet determined

/skill:arckit-nl-bio BIO2 conformance for 001, cloud-hosted case management system, cross-reference existing clause 4.6 encryption assessment

/skill:arckit-nl-bio Conformance assessment for an interdepartmental data-exchange platform in scope of the Cyberbeveiligingswet as an essential entity
```
...
- `/skill:arckit-nl-cloud` -- Feed BIO2 conformance status and control gaps into the cloud data-location and encryption assessment *(when This system is cloud-hosted or cloud hosting is under consideration)*
- `/skill:arckit-nl-tbb` -- Determine the TBB category so BIV scores can prioritise which BIO2 controls matter most *(when BIV scores not yet available to prioritise this conformance assessment)*
- `/skill:arckit-risk` -- Reflect open BIO2 control gaps in the risk register *(when Gaps identified in this assessment are not yet reflected in ARC-*-RISK)*
