...
- **TBB** (Te Beschermen Belangen determination, from `/skill:arckit-nl-tbb`) — Extract: TBB category and BIV scores. Use this directly; do not re-derive a TBB category inside this command.
- **BIO2** (BIO2 Conformance Assessment, from `/skill:arckit-nl-bio`) — Extract: existing conformance status relevant to the cloud environment
...
1. **Integral risk assessment**, required for materieel cloudgebruik, based on the BIV/TBB classification (pull from `/skill:arckit-nl-tbb` output if available; if not available, warn and proceed on user-supplied classification)
...
State that an exit plan is mandatory, documented, self-tested, covers two scenarios (planned exit; disruptive interruption of service), addresses destruction of data at the provider after migration, and is reviewed annually. Point to `/skill:arckit-nl-exit` for the full plan rather than duplicating it here.
...
Next steps:
1. {If TBB category not determined: Run /skill:arckit-nl-tbb to determine the TBB category}
2. {If materieel cloudgebruik and no exit plan: Run /skill:arckit-nl-exit for the clause 3.2 exit plan}
3. Run /skill:arckit-nl-bio for BIO2 conformance of the hosting environment
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
- **The TBB inference runs one way**: This command consumes a TBB category from `/skill:arckit-nl-tbb`; it does not infer a rubricering level from a TBB category. See that command's one-way inference warning.
...
- ✅ Clause 5.2 eligibility explicitly determined using a TBB category (or explicitly flagged as pending `/skill:arckit-nl-tbb`)
...
```text
/skill:arckit-nl-cloud Assess cloud eligibility for a case management platform at a Dutch ministry, processing personal data at scale, no cloud provider selected yet

/skill:arckit-nl-cloud Cloud hosting assessment for 001, departmental email and document collaboration service currently under review for public cloud migration

/skill:arckit-nl-cloud Evaluate materieel cloudgebruik and clause 5.2 eligibility for a system handling Departementaal VERTROUWELIJK policy documents
```
...
- `/skill:arckit-nl-tbb` -- Determine the TBB category / VIRBI 2025 rubricering for the data in scope before finalising the clause 5.2 eligibility check *(when TBB category not yet determined for the data processed by this system)*
- `/skill:arckit-nl-exit` -- Produce the mandatory clause 3.2 exit plan once materieel cloudgebruik is confirmed *(when Materieel cloudgebruik determined and no exit plan exists yet)*
- `/skill:arckit-nl-bio` -- Assess BIO2 conformance for the hosting environment and security controls *(when BIO2 baseline assessment not yet documented for this system)*
- `/skill:arckit-risk` -- Integrate cloud sovereignty, foreign-interference, and provider-concentration risks into the risk register *(when Risks identified in this assessment are not yet reflected in ARC-*-RISK)*
