...
# /skill:arckit-operationalize - Operational Readiness Command
...
Use `/skill:arckit-operationalize` after completing:

1. Requirements (`/skill:arckit-requirements`) - for SLA targets
2. Architecture diagrams (`/skill:arckit-diagram`) - for component inventory
3. HLD/DLD review (`/skill:arckit-hld-review` or `/skill:arckit-dld-review`) - for technical details
4. Data model (`/skill:arckit-data-model`) - for data dependencies

Run this command **before go-live** to ensure operational readiness. This is complementary to `/skill:arckit-servicenow` (which focuses on ITSM tooling) - this command focuses on the operational practices and documentation.
...
> **Tip**: Users can customize templates with `/skill:arckit-customize operationalize`
...
"⚠️ Cannot find requirements document (ARC-*-REQ-*.md). Please run `/skill:arckit-requirements` first. Operational readiness requires NFRs for SLO definitions."

### If No Architecture Diagrams

"⚠️ Cannot find architecture diagrams. Runbooks require component inventory. Please run `/skill:arckit-diagram container` first."
...
```markdown
---
**Generated by**: ArcKit `/skill:arckit-operationalize` command
**Generated on**: [DATE]
