...
> **Tip**: Users can customize templates with `/skill:arckit-customize pages`
...
- First line: `<!-- Generated by ArcKit /skill:arckit-pages -->` (generation marker)
- H1: repository name
...
**Skip logic**: Before writing, use the Read tool to check if `docs/llms.txt` exists. If it exists and does NOT contain the string `<!-- Generated by ArcKit /skill:arckit-pages -->`, it is hand-curated — do not overwrite it. Note this in the Step 5 summary.
...
Health Integration:
- Run `/skill:arckit-health JSON=true` to generate docs/health.json
- Re-run `/skill:arckit-pages` to display health data on the dashboard
