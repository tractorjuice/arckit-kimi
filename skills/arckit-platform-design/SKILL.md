...
- **PRIN** (Architecture Principles, in 000-global) — Extract: Platform governance principles, ecosystem orchestration standards, technology choices
  - If missing: STOP — platform designs require architecture principles. Run `/skill:arckit-principles` first.
...
- **STKE** (Stakeholder Analysis) — Extract: Ecosystem entities from stakeholder drivers, user personas, goals
  - If missing: recommend running `/skill:arckit-stakeholders` for better entity portraits
...
- `/skill:arckit-principles` - Architecture principles
- `/skill:arckit-stakeholders` - Stakeholder analysis (highly recommended)
- `/skill:arckit-requirements` - Platform requirements (recommended)
- `/skill:arckit-wardley` - Value chain analysis (recommended)

**Next Steps** (run after platform design):
- `/skill:arckit-sow` - RFP for platform development vendors
- `/skill:arckit-hld-review` - Review high-level platform architecture
- `/skill:arckit-backlog` - Generate sprint backlog from platform features
