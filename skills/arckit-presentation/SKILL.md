...
> **Tip**: Users can customize templates with `/skill:arckit-customize presentation`
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/skill:arckit-presentation` command | [PENDING] | [PENDING] |
```

#### Step 7e: Populate Generation Metadata Footer

```markdown
**Generated by**: ArcKit `/skill:arckit-presentation` command
...
User: `/skill:arckit-presentation Generate executive presentation for NHS appointment booking`
...
- If no artifacts exist, recommend running `/skill:arckit-plan` or `/skill:arckit-requirements` first
...
- Run `/skill:arckit-story` for a full narrative companion document
```
