...
> **Tip**: Users can customize templates with `/skill:arckit-customize principles-compliance`
...
if [ ! -f projects/000-global/ARC-000-PRIN-*.md ]; then
    ERROR "Architecture principles not found. Run /skill:arckit-principles first."
fi
```
...
**Generated by**: ArcKit `/skill:arckit-principles-compliance` command
...
After generating the assessment document, **suggest follow-up commands**:
   ...
   ```text
   📋 **Related Commands**:
   - /skill:arckit-analyze - Run comprehensive gap analysis
   - /skill:arckit-hld-review - Review vendor HLD against principles
   - /skill:arckit-dld-review - Review vendor DLD against principles
   - /skill:arckit-service-assessment - GDS Service Standard assessment (UK Gov)
   ```
