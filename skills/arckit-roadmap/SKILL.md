...
- **PRIN** (Architecture Principles, in `projects/000-global/`)
  ...
  - If missing: STOP and ask user to run `/skill:arckit-principles` first. The roadmap must align to approved principles.
...
> **Tip**: Users can customize templates with `/skill:arckit-customize roadmap`
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/skill:arckit-roadmap` command | [PENDING] | [PENDING] |
```
...
```markdown
**Generated by**: ArcKit `/skill:arckit-roadmap` command
...
4. Create detailed project plan for Phase 1: `/skill:arckit-plan`
5. Prepare business case (SOBC): `/skill:arckit-sobc`
6. Generate backlog from roadmap: `/skill:arckit-backlog`
...
7. **Integration**: Roadmap feeds into:
   - `/skill:arckit-plan` - Detailed plans for each phase
   - `/skill:arckit-sobc` - Business case aligned to roadmap investment
   - `/skill:arckit-backlog` - User stories prioritized by roadmap timeline
   - `/skill:arckit-traceability` - Full traceability matrix
...
- `/skill:arckit-backlog` -- Generate product backlog from roadmap
- `/skill:arckit-plan` -- Create detailed project plan for Phase 1
