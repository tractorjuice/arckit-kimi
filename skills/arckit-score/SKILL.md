...
1. **Read the project's EVAL artifact** (evaluation criteria):
   - If no EVAL exists, tell the user to run `/skill:arckit-evaluate` first
...
2. **Read vendor proposal** from `projects/{id}/vendors/{vendor-name}/`:
   ...
   **objective evidence** for the relevant criterion's score, carrying the caveat that awarded value is not
   actual spend (it evidences track record, not committed cost). Run `/skill:arckit-competitors --supplier
...
- `/skill:arckit-evaluate` -- Create or update evaluation framework before scoring *(when No EVAL artifact exists for the project)*
- `/skill:arckit-sow` -- Generate Statement of Work for selected vendor *(when Vendor selection complete, ready for procurement)*
- `/skill:arckit-competitors` -- Generate the vendor's UK award-history evidence for scoring *(when UK government procurement context)*
