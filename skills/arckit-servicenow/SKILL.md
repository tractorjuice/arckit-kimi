...
# /skill:arckit-servicenow - ServiceNow Service Design Command
...
Use `/skill:arckit-servicenow` after completing:

1. Requirements (`/skill:arckit-requirements`)
2. Architecture diagrams (`/skill:arckit-diagram`) - especially C4 diagrams
3. High-Level Design (HLD) or Detailed Design (DLD) - if available
4. Technology Code of Practice assessment (`/skill:arckit-tcop`) - for UK Gov projects
...
- **DIAG** (Architecture Diagrams) in `projects/{project-name}/diagrams/`
  ...
  - If missing: warn user to run `/skill:arckit-diagram` first
...
> **Tip**: Users can customize templates with `/skill:arckit-customize servicenow`
...
```text
/skill:arckit-servicenow Generate ServiceNow design for the DWP Benefits Eligibility Chatbot - this is a Tier 1 critical service requiring 24/7 support
```
...
```text
/skill:arckit-servicenow Create ServiceNow design for the payment processing service
```
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/skill:arckit-servicenow` command |
```

### Generation Metadata Footer

```markdown
**Generated by**: ArcKit `/skill:arckit-servicenow` command
**Generated on**: {DATE}
...
"⚠️ Cannot find requirements document (ARC-*-REQ-*.md). Please run `/skill:arckit-requirements` first. ServiceNow design requires NFRs for SLA definitions."

### If No Architecture Diagrams Found

"⚠️ Cannot find architecture diagrams. Please run `/skill:arckit-diagram context` and `/skill:arckit-diagram container` first. ServiceNow design requires architecture diagrams for CMDB structure."
