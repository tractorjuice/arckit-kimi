...
- **After**: `/skill:arckit-stakeholders` (MANDATORY - SOBC must link to stakeholder goals)
- **Before**: `/skill:arckit-requirements` (SOBC justifies whether to proceed with detailed requirements)
...
1. **Read existing artifacts from the project context:**
   ...
   **MANDATORY** (warn if missing):
   - **STKE** (Stakeholder Analysis) in `projects/{project}/`
     ...
     - If missing: STOP and warn user to run `/skill:arckit-stakeholders` first — every SOBC benefit MUST trace to a stakeholder goal
...
7. **Generate comprehensive SOBC**:
   ...
   **Read the template** (with user override support):
   ...
   > **Tip**: Users can customize templates with `/skill:arckit-customize sobc`
...
9. **Include decision framework**:
   ...
   - **Next Steps**: If approved, what happens next?
     - Typically: `/skill:arckit-requirements` to define detailed requirements
     - Then: `/skill:arckit-business-case-detailed` with accurate costs
...
3. **Next steps**:
   ...
   - "If approved: Run `/skill:arckit-requirements` to define detailed requirements"
...
- Tell user: "SOBC requires stakeholder analysis to link benefits to stakeholder goals. Please run `/skill:arckit-stakeholders` first."

If user wants detailed business case:

- Tell user: "This command creates SOBC (Strategic Outline Business Case) - the first stage with high-level estimates. After `/skill:arckit-requirements`, create OBC (Outline Business Case) with refined costs. After design, create FBC (Full Business Case) for final approval."
...
To avoid exceeding Kimi output limits, you MUST use the following strategy:
...
- `/skill:arckit-requirements` -- Define detailed requirements after SOBC approval
- `/skill:arckit-roadmap` -- Create strategic roadmap from SOBC investment plan
- `/skill:arckit-tenders` -- Anchor the Economic Case market view with real UK award-value benchmarks *(when UK government procurement context)*
