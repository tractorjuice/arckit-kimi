...
3. **Read the template** (with user override support):
   ...
   > **Tip**: Users can customize templates with `/skill:arckit-customize sow`
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/skill:arckit-sow` command | [PENDING] | [PENDING] |
```
...
- Major deliverables
- Suggested timeline
- Next steps (e.g., "Now run `/skill:arckit-evaluate` to create vendor evaluation framework")

## Example Usage

User: `/skill:arckit-sow Generate SOW for payment gateway modernization project`
...
- Run `/skill:arckit-evaluate` to create vendor evaluation framework
```
...
- `/skill:arckit-evaluate` -- Create vendor evaluation framework
- `/skill:arckit-dos` -- Generate Digital Marketplace DOS opportunity
