...
2. **Read the template** (with user override support):
   ...
   > **Tip**: Users can customize templates with `/skill:arckit-customize stakeholder-drivers`
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/skill:arckit-stakeholders` command |
```

### Generation Metadata Footer

```markdown
**Generated by**: ArcKit `/skill:arckit-stakeholders` command
**Generated on**: {DATE}
...
8. **Summarize what you created**:
   ...
   - Suggested next steps: "Now run `/skill:arckit-requirements` to create requirements that align with and address these stakeholder goals and drivers"
...
```text
/skill:arckit-stakeholders Analyze stakeholders for a cloud migration project where the CFO wants cost savings, the CTO wants innovation, Operations is worried about downtime, and Security needs enhanced controls
```
...
```text
/skill:arckit-stakeholders Analyze stakeholders for a DWP benefits chatbot where the Minister wants quick delivery, Civil Service wants due diligence, Citizens need accuracy, and ICO requires data protection
```
...
- `/skill:arckit-requirements` -- Create requirements aligned to stakeholder goals
- `/skill:arckit-risk` -- Create risk register with stakeholder risk owners
- `/skill:arckit-sobc` -- Build business case from stakeholder drivers
