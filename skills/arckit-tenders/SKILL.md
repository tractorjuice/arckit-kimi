...
Unlike `/skill:arckit-datascout`, requirements are **not** mandatory here. If
...
- Next steps (`/skill:arckit-sobc`, `/skill:arckit-risk`, `/skill:arckit-research`).

## Edge Cases

- **No requirements**: not a failure here. Proceed with the explicit
  `$ARGUMENTS` scope and say so. (`/skill:arckit-datascout` requires requirements;
...
- `/skill:arckit-sobc` -- Anchor the Economic Case with real median award values
- `/skill:arckit-risk` -- Record supplier-concentration / single-supplier-dependency risk
- `/skill:arckit-research` -- Build-vs-buy market context
