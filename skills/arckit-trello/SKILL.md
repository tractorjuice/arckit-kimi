...
Reads the JSON backlog produced by `/skill:arckit-backlog FORMAT=json` and pushes it to Trello:
...
- This is produced by `/skill:arckit-backlog FORMAT=json`
...
Please generate one first:
  /skill:arckit-backlog FORMAT=json

Then re-run /skill:arckit-trello
```
...
- `/skill:arckit-backlog FORMAT=json` - Backlog JSON file (MANDATORY)
