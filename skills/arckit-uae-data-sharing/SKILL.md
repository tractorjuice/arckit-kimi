...
- Every share involving personal data MUST cite a PDPL lawful basis. Where `/skill:arckit-uae-pdpl` has not yet been run, recommend it as a follow-up before sign-off.
...
- `/skill:arckit-platform-design` -- Federation and API mechanisms feed the integration specification.
- `/skill:arckit-uae-pdpl` -- Per-share PDPL lawful basis must be reflected in the PDPL compliance assessment.
