...
**Generated by**: ArcKit `/skill:arckit-uk-fs-consumer-duty` command
**Generated on**: [DATE]
...
- `/skill:arckit-requirements` -- Fair-value framework expresses as NFRs in the requirements artefact.
- `/skill:arckit-stakeholders` -- Target market assessment feeds Goals + stakeholder traceability.
- `/skill:arckit-dpia` -- Price-personalisation or vulnerable-customer scoring involves profiling — DPIA required.
- `/skill:arckit-risk` -- Foreseeable harms map to Orange Book risk register entries.
- `/skill:arckit-uk-fs-sca-rts` -- SCA biometric/accessibility design intersects with Consumer Understanding outcome for vulnerable customers.
- `/skill:arckit-uk-fs-safeguarding` -- EMI/PI redemption accessibility under Consumer Support outcome intersects with safeguarding obligations.
