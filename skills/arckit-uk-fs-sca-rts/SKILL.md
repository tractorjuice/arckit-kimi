...
**Generated by**: ArcKit `/skill:arckit-uk-fs-sca-rts` command
**Generated on**: [DATE]
...
- `/skill:arckit-uk-fs-safeguarding` -- PSP scope often overlaps EMI scope — if the firm is also issuing e-money, safeguarding is a parallel obligation.
- `/skill:arckit-dpia` -- SCA design involves biometrics, device fingerprinting, and behavioural data — DPIA is required.
- `/skill:arckit-adr` -- Exemption application choices are architectural and should be recorded as ADRs for traceability.
- `/skill:arckit-risk` -- SCA exemption misapplication maps to fraud-loss and regulatory-enforcement risk register entries.
