...
ICAM is the foundation of the Zero Trust Identity pillar — feed IAL/AAL/FAL outputs into `/skill:arckit-us-zero-trust` scoring. The proofing data flows (especially at IAL2/IAL3) collect significant PII, so trigger `/skill:arckit-us-privacy-pia`. Identity-provider selection and federation pattern decisions should be captured as ADRs via `/skill:arckit-adr`.
...
- `/skill:arckit-us-zero-trust` -- ICAM is the foundation of the Zero Trust Identity pillar; IAL/AAL/FAL selections directly score ZTMM Identity functions.
- `/skill:arckit-us-privacy-pia` -- Identity proofing collects and processes PII (especially IAL2/IAL3); the ICAM data flows feed the PIA personal-information inventory.
- `/skill:arckit-adr` -- Identity provider selection (PIV vs login.gov vs agency-specific) and federation pattern decisions warrant ADRs.
