...
1. Read prerequisites:
   ...
   - **REQUIRED**: The project's FIPS 199 artefact — the system water-mark selects the baseline. If absent, stop and direct the user to run `/skill:arckit-us-fisma-categorization` first.
...
The tailored control matrix is the source-of-truth for the FedRAMP SSP (`/skill:arckit-us-fedramp-ssp`) and the input to the Zero Trust scoring (`/skill:arckit-us-zero-trust`). The SR (Supply Chain Risk Management) family controls cross-reference the EO 14028 attestation and SBOM via `/skill:arckit-us-sbom-eo-14028`. Any non-trivial compensating controls, inheritance-boundary calls, or parameter-value deviations should be captured as ADRs via `/skill:arckit-adr`.
...
- `/skill:arckit-us-fedramp-ssp` -- The tailored control set and implementation statements drop directly into the FedRAMP SSP control-implementation tables.
- `/skill:arckit-us-zero-trust` -- Control selections (especially AC, IA, SC families) feed the CISA Zero Trust Maturity Model scoring.
- `/skill:arckit-us-sbom-eo-14028` -- Supply-chain controls (SR family) cross-reference the EO 14028 secure-software attestation and SBOM register.
- `/skill:arckit-adr` -- Significant tailoring decisions (compensating controls, control inheritance boundaries, parameter values) warrant ADRs.
