...
The attestation and SBOM directly evidence NIST 800-53 SR and SA family controls in `/skill:arckit-us-nist-800-53`. SBOM-format choice, signing strategy, and any exception requests should be captured as ADRs via `/skill:arckit-adr`. Components with known unmitigated vulnerabilities and any open M-22-18 exceptions feed `/skill:arckit-risk`.
...
- `/skill:arckit-us-nist-800-53` -- SR (Supply Chain Risk Management) and SA (System and Services Acquisition) control family implementations must cross-reference the attestation and SBOM.
- `/skill:arckit-adr` -- SBOM format choice (CycloneDX vs SPDX), signing strategy (Sigstore, in-toto, SLSA level), and attestation exception requests warrant ADRs.
- `/skill:arckit-risk` -- Components with known unmitigated vulnerabilities or attestation exceptions feed the risk register.
