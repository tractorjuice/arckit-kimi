...
- **WARD** (Wardley Map, in `projects/{project}/wardley-maps/`) — Extract: all components with evolution positions, dependencies, inertia points, current stage classifications, map title and strategic question
  - If missing: warn user — "A Wardley Map (WARD artifact) is required before running climate assessment. Please run `/skill:arckit-wardley` first to create your map."
...
> **Tip**: Users can customise templates with `/skill:arckit-customize wardley.climate`
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/skill:arckit-wardley-climate` command | [PENDING] | [PENDING] |
```

**Populate Generation Metadata Footer**:

```markdown
**Generated by**: ArcKit `/skill:arckit-wardley-climate` command
...
/skill:arckit-wardley — Create Wardley Map (required WARD artifact)

# Analysis layer: run after map creation
/skill:arckit-wardley-climate — Assess climatic patterns (WCLM artifact) ← you are here
/skill:arckit-wardley-gameplay — Select gameplays informed by climate forces (WGAM artifact)

# Execution layer: run after analysis
/skill:arckit-roadmap — Create execution roadmap
/skill:arckit-strategy — Synthesise into architecture strategy
```
...
```bash
"A Wardley Map is required. Run `/skill:arckit-wardley` first to create your map, then return here."
```
...
evidence. Consider running `/skill:arckit-research` to gather vendor landscape and market dynamics data,
...
This is the most urgent strategic scenario. Run `/skill:arckit-wardley-gameplay` immediately to identify
...
before gameplay execution plans can succeed. Consider running `/skill:arckit-strategy` to create
...
Consider running `/skill:arckit-wardley` to update component positions, then re-run gameplay analysis."
...
strategy. Run `/skill:arckit-tcop` to validate compliance positioning."
```
...
Next Steps:
- /skill:arckit-wardley-gameplay — Select plays informed by this climate assessment
- /skill:arckit-wardley — Update map if evolution velocity findings change component positions
- /skill:arckit-strategy — Synthesise climate findings into architecture strategy
```
...
- `/skill:arckit-wardley-gameplay` -- Select gameplays informed by climate forces
- `/skill:arckit-wardley` -- Update map with climate-driven evolution predictions *(when Climate analysis reveals evolution velocity changes)*
