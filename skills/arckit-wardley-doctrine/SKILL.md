...
- **PRIN** (Architecture Principles, in `000-global`) — Extract: Stated principles, governance standards, technology standards, decision-making approach, values. Principles reveal what the organization believes it does; doctrine assessment reveals what it actually does.
  - If missing: warn the user. Doctrine assessment is still possible from other artifacts and user input, but principles would significantly improve accuracy. Recommend running `/skill:arckit-principles` for the global project first.
...
> **Tip**: Users can customize templates with `/skill:arckit-customize wardley-doctrine`
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial doctrine assessment from `/skill:arckit-wardley-doctrine` command | [PENDING] | [PENDING] |
```

**Populate Generation Metadata Footer**:

```markdown
**Generated by**: ArcKit `/skill:arckit-wardley-doctrine` command
...
- /skill:arckit-wardley — Create or refine Wardley Map informed by doctrine gaps
- /skill:arckit-wardley-gameplay — Select gameplays that address doctrine weaknesses
- /skill:arckit-principles — Review and update architecture principles to reflect doctrine findings
```
...
- `/skill:arckit-wardley` -- Create or refine Wardley Map informed by doctrine gaps *(when Doctrine gaps affect component positioning or strategy)*
- `/skill:arckit-wardley-gameplay` -- Select gameplays that address doctrine weaknesses
