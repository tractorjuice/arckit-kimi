...
- **WARD** (Wardley Map, in `projects/{project}/wardley-maps/`) — Extract: all components with evolution positions, dependencies, inertia points, build/buy decisions already made, map title and strategic question
  - If missing: warn user — "A Wardley Map (WARD artifact) is required before running gameplay analysis. Please run `/skill:arckit-wardley` first to create your map."
...
> **Tip**: Users can customise templates with `/skill:arckit-customize wardley.gameplay`
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/skill:arckit-wardley-gameplay` command | [PENDING] | [PENDING] |
```

**Populate Generation Metadata Footer**:

```markdown
**Generated by**: ArcKit `/skill:arckit-wardley-gameplay` command
...
/skill:arckit-wardley-climate — Assess climatic patterns (WCLM artifact)
/skill:arckit-wardley-gameplay — Identify strategic plays (WGAM artifact) ← you are here

# Execution layer: run after analysis
/skill:arckit-roadmap — Create roadmap to execute selected plays
/skill:arckit-strategy — Synthesise into architecture strategy document
```
...
```bash
"Note: No climate assessment (WCLM) found. Consider running `/skill:arckit-wardley-climate` after this analysis —
...
"Your selected plays include resource-intensive options. Consider running `/skill:arckit-roadmap` to create a
phased execution plan with investment milestones."

# If ecosystem plays selected (Tower and Moat, N-sided Markets, etc.)
"Your strategy includes ecosystem plays. Consider running `/skill:arckit-strategy` to synthesise these into
...
`/skill:arckit-sow` if procurement is needed for the ecosystem, or `/skill:arckit-research` to identify
...
(Open Source), Point 8 (Share/Reuse), and G-Cloud procurement constraints. Run `/skill:arckit-tcop`."
```
...
Next Steps:
- /skill:arckit-wardley-climate — Validate plays against climatic forces (if not done)
- /skill:arckit-roadmap — Create execution roadmap for selected plays
- /skill:arckit-strategy — Synthesise gameplay into architecture strategy
```
...
- `/skill:arckit-roadmap` -- Create roadmap to execute selected plays
- `/skill:arckit-strategy` -- Synthesise gameplay into architecture strategy
- `/skill:arckit-wardley-climate` -- Validate plays against climatic patterns *(when Climate assessment not yet performed)*
