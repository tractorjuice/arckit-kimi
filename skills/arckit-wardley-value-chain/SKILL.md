...
- **REQ** (Requirements) — Extract: User needs, business requirements, functional capabilities, system components, integration points. Anchor the value chain in genuine user needs, not internal assumptions.
  - If missing: warn the user and recommend running `/skill:arckit-requirements` first. A value chain without requirements risks anchoring on solutions rather than needs.
...
> **Tip**: Users can customize templates with `/skill:arckit-customize wardley-value-chain`
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/skill:arckit-wardley-value-chain` command | [PENDING] | [PENDING] |
```

**Populate Generation Metadata Footer**:

```markdown
**Generated by**: ArcKit `/skill:arckit-wardley-value-chain` command
...
🎯 Next Steps:
- Run /skill:arckit-wardley to create a full Wardley Map using this value chain
...
- /skill:arckit-wardley — Create full Wardley Map with evolution axis positioning
- /skill:arckit-wardley-doctrine — Assess organizational maturity to execute the strategy
- /skill:arckit-requirements — Strengthen the requirements before refining the chain (if incomplete)
```
...
- `/skill:arckit-wardley` -- Create Wardley Map from this value chain
- `/skill:arckit-wardley-doctrine` -- Assess organizational doctrine maturity *(when Value chain reveals organizational capability gaps)*
