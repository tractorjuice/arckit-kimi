...
- **PRIN** (Architecture Principles, in 000-global) — Extract: Strategic principles, technology standards, compliance requirements, cloud-first policy
  - If missing: warn user to run `/skill:arckit-principles` first
- **REQ** (Requirements) — Extract: Business requirements, functional requirements, non-functional requirements. Identify: User needs, capabilities, technical components
  - If missing: warn user to run `/skill:arckit-requirements` first
...
> **Tip**: Users can customize templates with `/skill:arckit-customize wardley`
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/skill:arckit-wardley` command | [PENDING] | [PENDING] |
```
...
```markdown
**Generated by**: ArcKit `/skill:arckit-wardley` command
...
"I recommend running `/skill:arckit-requirements` first to establish requirements before creating a Wardley Map."

# If no architecture principles exist
"I recommend running `/skill:arckit-principles` first to establish architecture principles."
```
...
"Based on your Wardley Map, I recommend running `/skill:arckit-sow` to create an RFP for vendor procurement."

# If map shows build vs buy decisions needed
"Your map identifies several build vs buy decisions. Consider running `/skill:arckit-evaluate` to compare vendor options."

# If map shows UK Government project
"As a UK Government project, I recommend running `/skill:arckit-tcop` to assess Technology Code of Practice compliance."

# If map shows AI components with HIGH-RISK
"Your map includes HIGH-RISK AI components. I recommend running `/skill:arckit-ai-playbook` and `/skill:arckit-atrs`."
```
...
```bash
"/skill:arckit-hld-review Review HLD against Wardley Map strategic positioning"
```
...
The `/skill:arckit-analyze` command should include Wardley Map validation:
...
1. Run `/skill:arckit-sow` to create RFP for G-Cloud procurement (AWS, GPT-4, Auth0)
2. Run `/skill:arckit-ai-playbook` to complete AI Playbook assessment
3. Run `/skill:arckit-atrs` to generate ATRS record
4. Run `/skill:arckit-tcop` to validate TCoP compliance (Cloud First, Open Standards, Reuse)
...
- /skill:arckit-sow - Generate RFP for vendor procurement
- /skill:arckit-tcop - Assess Technology Code of Practice compliance
- /skill:arckit-ai-playbook - Assess AI Playbook compliance (if AI components)
```
...
- `/skill:arckit-roadmap` -- Create strategic roadmap from evolution analysis
- `/skill:arckit-strategy` -- Synthesise Wardley insights into architecture strategy
- `/skill:arckit-research` -- Research vendors for Custom-Built components *(when Custom-Built components identified that need market research)*
- `/skill:arckit-wardley-doctrine` -- Assess organizational doctrine maturity
- `/skill:arckit-wardley-gameplay` -- Identify strategic plays from the map
- `/skill:arckit-wardley-climate` -- Assess climatic patterns affecting components
